...
if os.path.exists(config['GENOME_STORE_FILE_PATH']):
    user_response = input('Indexing Genome store {}. Continue? Y/N:'.format(config['GENOME_STORE_FILE_PATH']))
...
if os.path.exists(config['INDEX_FILE_PATH']):
    user_response = input('Append to existing indexes? Y/N:'.format(config['INDEX_FILE_PATH']))
    if user_response.lower().startswith("y"):
        with open(config['INDEX_FILE_PATH'], 'r') as index_file:
...
with open(config['INDEX_FILE_PATH'], 'w') as write_file:
